// ==UserScript==
// @name         chess Piece Customizer 2.9.4
// @namespace    https://lichess.org/
// @version      2.9.4
// @description  chess pieces with custom images, including color-specific pawns for the player. Restored greeting and last-move-highlighting features. Now includes time alerts at 40, 30, 20, and 10 seconds remaining.
...
// @description  - Sends sound and visual alerts when the player's clock hits 40, 30, 20, and 10 seconds.
...
(function() {
    ...
    // --- Eigene Turm-Bilder (NEU HINZUGEFÜGT) ---
    const DEFAULT_black_rook_URL = 'https://svgsilh.com/svg/307015.svg';
    const DEFAULT_white_rook_URL = 'https://svgsilh.com/svg/307015-795548.svg';


    // black geopmetry bishop
    // white geopmetry bishop
    const DEFAULT_black_bishop_URL = 'https://lichess.org/assets/hashed/bB.4fb6f771.svg';
    const DEFAULT_white_bishop_URL = 'https://lichess1.org/assets/hashed/wB.a6dd62c3.svg';
    ...
    function replacePieceImages(pieceConfig) {
        ...
        allPiecesOnBoard.forEach(pieceElement => {
            ...
            if (pieceElement.classList.contains(playerColor)) {
                ...
                // 1. Spezielle Behandlung für Bauern (Immer eigene Bilder)
                ...
                // 2. Spezielle Behandlung für Türme (Rooks) (Immer eigene Bilder, wie Bauern)
                } else if (pieceElement.classList.contains('rook')) {
                    const rookUrl = (playerColor === 'white') ? DEFAULT_white_rook_URL : DEFAULT_black_rook_URL;
                    applyCustomImage(pieceElement, rookUrl);
                    isCustomized = true;
                } else if (pieceElement.classList.contains('bishop')) {
                    const bishopUrl = (playerColor === 'white') ? DEFAULT_white_bishop_URL : DEFAULT_black_bishop_URL;
                    applyCustomImage(pieceElement, bishopUrl);
                    isCustomized = true;


                    // 3. Behandlung der restlichen Hauptfiguren (K, Q, B, N) basierend auf URL-Parametern
                } else {
                    for (const pieceType in pieceConfig.player) {
                        const config = pieceConfig.player[pieceType];
                        // Türme wurden oben bereits behandelt, daher überspringen wir die R-Konfiguration hier.
                        if (config.type === 'rook') continue;
                ...
                // Die Logik für die Gegnerfiguren bleibt unverändert und verwendet die URL-Parameter
